    ...
    while not sms.sms_getter_stop_event.is_set():
        try:
            ...
            if whatsapp_code:
                ...
                notification = f"✅ *OTP Acquired! (Real-time Fetch)*\n\n*Number:* `{phone_number_to_watch}`\n*OTP:* `{whatsapp_code}`"
        ...
        try:
            ...
            for update in updates:
                ...
                if is_group and text.isdigit() and len(text) > 8:
                    ...
                    if cached_otp:
                        reply = f"✅ @{username}, cached code for `{text}` is: `{cached_otp}`"
                    ...
                    reply = f"❌ @{username}, code not received for `{text}`"
                    if data_sms.get('data'):
                        for sms_entry in data_sms['data']:
                            ...
                            if sms_number == text:
                                ...
                                if otp_code:
                                    reply = f"✅ @{username}, code for `{text}` is: `{otp_code}`"
                                    with sms.otp_cache_lock:
